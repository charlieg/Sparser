...
;; 1.2 (12/7) reworked the threading so that known marked-capitalization case
;;      would get priority. 12/13 cont. to allow entry-points that don't consider
;;      unknown words.  12/29 added traces.
;; 1.3 (1/25/94) presumption that the existence of a caps variant ipso facto
;;      meant it was known proved false.
;; 1.4 (2/26) fixed a presumption about the existence of an rs when the call
;;      started in PFWPNF.
      ...
      (if relevant-variant
        (then
          ...
          (if (word-rules relevant-variant)
            (check-known-word-for-word-fsas relevant-variant
                                            (word-rules relevant-variant)
                                            position)
            (else
              ;; //trace - variant, but unknown
              (dispatch-off-capitalization-data actual-state
                                                relevant-variant
                                                position))))
        (else
          (tr :wfsa/no-variants)
