...
;; 0.3 (1/17) put in long thread for verb groups beginning with 'be' and having no
;;      other verb.  1/23 fixed glitch in MVG/adv-check-before-mvb.  1/24 fixed
;;      that path's return values.
;; 0.4 (1/26) changed call in mine-indeterminate.. with one word to anticipate that
  ...
  (let ((form (edge-form next-tt)))
    (ecase (cat-symbol form)
      ...
      (category::infinitive
       (list (edge-referent next-tt)))
      (category::adverb
       (mvg/aux-adv-mvb/edge (next-treetop/rightward 
  ...
  (let ((next-pos starts-at)
    ...
    (loop
      ...
      (setq term
            (etypecase tt
              (edge
               (cond ((polyword-p (edge-category tt))
                      (mine-unmarked-term/pw tt))
                     ((eq (edge-left-daughter tt) word::and-sign)
                      ;; more generally, any punctuation that is allowed
                      ;; into a sequence by PNF.  We can't just make
                      ;; this a term because it's not legal to do that.
                      (let ((nw (name-word-for-word
                                 (edge-left-daughter tt))))
                        (if nw
                          nw
                          (break "The word \"&\" should have had a ~
                                  name-word defined for it"))))
                      
                     ((edge-over-literal? tt)
                      (mine-unmarked-term-under-edge tt))
                     (t (edge-referent tt))))
              (word
               (mine-unmarked-term
