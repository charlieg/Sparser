...
(export '(Assimilate-dis.out
          ))

(defparameter *trace-.dis-interface* nil
  "Flag controlling occasional format statements that let you
   keep track of how the analysis of a file is progressing.")
...
(defun assimilate-special-token (multi-token-string)
  ...
  (let ((polyword (sparser::polyword-named multi-token-string)))
    ...
    (when *trace-.dis-interface*
      (format t "~A " (sparser::word-pname polyword)))
