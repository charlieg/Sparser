...
(defun find-sequence (items category)
  (let ((instances (cat-instances category::sequence))
        ...
        (count (length items)))
    (when instances
      (let ((count-entry (gethash count instances)))
        ;; check for the right number of entries
        (when count-entry
          (let ((category-entry (cdr (assoc category count-entry
                                            :test #'eq))))
            (when category-entry
              (let ((first-item-entry (cdr (assoc first-item category-entry
                                                  :test #'eq))))
                (when first-item-entry
                  (if (null (cdr first-item-entry))
                    (when (equal items (value-of 'items (first first-item-entry)))
                      (first first-item-entry))
                    (dolist (sequence first-item-entry nil)
                      (when (equal items (value-of 'items sequence))
                        (return sequence)))))))))))))
...
; Given the current assumption that explicitly defined sequences are fixed
; when they are created, we do brute force lookup and storage rather than
; use the lattice. Since it was done that way in the past, I'm indexing
; by count first, then by type, then list identity.

(defun index-sequence (sequence)
  (unless (cat-instances category::sequence)
    (setf (cat-instances category::sequence)
          (make-hash-table :test #'eql)))
  (let* ((instances (cat-instances category::sequence))
         (category (value-of 'type sequence))
         (first-item (first (value-of 'items sequence)))
         (count (value-of 'number sequence))
         (count-entry (gethash count instances)))
    (if count-entry
      (let ((category-entry (assoc category count-entry :test #'eq)))
        (if category-entry
          (let ((first-item-entry (assoc first-item (cdr category-entry)
                                         :test #'eq)))
            (if first-item-entry
              (then
                (rplacd first-item-entry
                        (cons sequence
                              first-item-entry))
                (break "added first-item entry"))
              (else
                (rplacd category-entry
                        (cons `(,first-item ,sequence)
                              (cdr category-entry))))))
          (else 
            (rplacd count-entry
                    (cons `(,category
      ...
      ;; First instance of a sequence of this length
      (let* ((entry `( (,category
                        (,first-item ,sequence)) )))
        (setf (gethash count instances) entry)))))
