...
;	   (break "define-individual 2 -- success"))
...
;      (format t "~&About to call apply bindings -- no-indexing~%")
...
(defun make/permanent-individual (category binding-instructions)
  ...
  (let ((*index-under-permanent-instances* t)
        (individual (make-individual :type `(,category)
                                     :id (next-id category))))
;    (format t "~&About to call apply bindings -- permanent~%")
    (let ((bindings
           (apply-bindings individual binding-instructions)))
...
(defun make-simple-individual (category binding-instructions)
  ...
  (let ((individual (if *index-under-permanent-instances*
                      ...
                      (allocate-individual))))
    (setq *i* individual)
;    (break "make-simple-individual -- able to move?  *i* = ~a" *i*) ;; yes, then
    (setf (indiv-type individual) (list category))   ;; immediately lost it
    (setf (indiv-id   individual) (next-id category))
;    (format t "~&About to call apply bindinggs -- simple~%")
    (let ((bindings
	   (apply-bindings individual binding-instructions)))
